...
async def async_setup_realtime_events(hass: HomeAssistant, entry, client, cfg: Dict[str, Any]) -> None:
    ...
    try:
        async_register(hass, DOMAIN, "SC-600", wid, _handle_webhook)
    except Exception:  # noqa: BLE001
        # If already registered, ignore
        _LOGGER.debug("Siedle SC-600: webhook already registered", exc_info=True)
    ...
    try:
        ...
        _LOGGER.debug("Siedle SC-600: could not fetch static event types", exc_info=True)
    ...
    try:
        if hasattr(client, "set_events_webhook"):
            ...
            _LOGGER.info("Siedle SC-600: realtime events enabled (webhook configured)")
    except Exception as err:  # noqa: BLE001
        _LOGGER.warning("Siedle SC-600: could not configure TechPoint webhook: %s", err)
