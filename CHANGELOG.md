...
### Fixed
- IO userdefined endpoint: `/config/io/userdefined/filter` returned 404,
  reverted to `/config/io/userdefined` with `{"ioFilter": {"ioType": N}}` body
- Coordinator now disables a failing optional fetch (areas, zones, IO,
  cardholders) for the rest of the session after first failure, so the
  TechPoint controller's error log no longer fills with repeated 400/404
  responses every poll cycle. Reset on integration reload.
